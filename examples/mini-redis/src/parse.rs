...
impl Parse {
    ...
    pub(crate) fn new(frame: Frame) -> Result<Parse, ParseError> {
        let array = match frame {
            ...
            frame => return Err(format!("lib_protocol error; expected array, got {:?}", frame).into()),
        };
    ...
    pub(crate) fn next_string(&mut self) -> Result<String, ParseError> {
        match self.next()? {
            ...
            Frame::Bulk(data) => str::from_utf8(&data[..])
                .map(|s| s.to_string())
                .map_err(|_| "lib_protocol error; invalid string".into()),
            frame => Err(format!(
                "lib_protocol error; expected simple frame or bulk frame, got {:?}",
                frame
            )
            .into()),
        }
    ...
    pub(crate) fn next_int(&mut self) -> Result<u64, ParseError> {
        ...
        const MSG: &str = "lib_protocol error; invalid number";
        ...
        match self.next()? {
            ...
            frame => Err(format!("lib_protocol error; expected int frame but got {:?}", frame).into()),
        }
    }
    ...
    pub(crate) fn finish(&mut self) -> Result<(), ParseError> {
        if self.parts.next().is_none() {
            ...
            Err("lib_protocol error; expected end of frame, but there was more".into())
        }
    }
}
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ParseError::EndOfStream => "lib_protocol error; unexpected end of stream".fmt(f),
